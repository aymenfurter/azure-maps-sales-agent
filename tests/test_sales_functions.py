...
from mock_api import OFFICE_LOCATION, SAMPLE_CLIENTS
...
def test_format_coordinates_for_azure_maps():
    ...
    # Create test clients
    test_clients = [
        {"coordinates": {"latitude": 47.1234, "longitude": 8.5678}},
        {"coordinates": {"latitude": 46.7890, "longitude": 9.1234}},
    ]
...
def test_get_clients_for_today_success(mock_get_todays_clients, reset_globals):
    ...
    mock_response = {"date": "2023-07-15", "office": OFFICE_LOCATION, "clients": SAMPLE_CLIENTS[:2]}  # First 2 clients
...
def test_plan_optimal_route_success(mock_get_key, mock_requests_get, mock_get_clients, reset_globals):
    ...
    mock_api_response = {
        "routes": [
            {
                "summary": {"lengthInMeters": 15000, "travelTimeInSeconds": 1800, "trafficDelayInSeconds": 120},
                "legs": [
                    {
                    ...
                    {
                        "summary": {"lengthInMeters": 10000, "travelTimeInSeconds": 1200},
                        "points": [],  # Would have many points in real response
                    },
                ],
                "sections": [],
...
def test_get_next_visit(mock_get_client_details, reset_globals):
    ...
    # Mock client details
    mock_client_details = {**SAMPLE_CLIENTS[0], "last_visit": "2023-06-15", "notes": "Interested in new product line"}
...
def test_generate_location_map_with_query(mock_get_key, mock_requests_get, reset_globals):
    ...
    geocode_response = {"results": [{"position": {"lat": 47.3698, "lon": 8.5392}}]}
